보내주신 모바일 야구 게임들의 UI 스타일과 실제 타석 시점 그래픽을 참고하여, **공수교대(이닝 전환) 시 플레이어가 직접 구종을 골라 투구할 수 있는 기능**과 **불펜 투수 교체 시스템**을 전면 추가했습니다!

이제 플레이어 팀이 수비할 때(이닝 말) 직접 투수가 되어 공을 던질 수 있으며, 선발 투수가 지치거나 위기에 처했을 때 불펜 투수를 호출할 수 있습니다.

### ⚾ 추가 및 변경된 핵심 기능

1. **플레이어 직접 투구 모드 (수비 이닝):** 수비 이닝이 되면 화면에 **[포심], [슬라이더], [체인지업]** 구종 선택 버튼이 나타납니다. 버튼을 누르면 해당 구종의 궤적과 속도로 공이 투구됩니다.
2. **불펜(투수 교체) 시스템:** 각 팀마다 **선발, 중간 계투, 마무리** 투수진이 라인업에 완벽하게 추가되었습니다. 경기 중 언제든 **[투수 교체]** 버튼을 눌러 불펜에 대기 중인 투수로 교체할 수 있습니다.
3. **선수단 데이터 전면 보강:** 제공해주신 이미지를 바탕으로 KBO 10개 구단의 베스트 라인업(선발/중간/마무리 투수 포함 및 구자욱, 강민호 등 주요 타자)을 완전하게 연동했습니다.
